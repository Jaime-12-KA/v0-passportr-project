...
import NewPassportModal from "@/components/modals/new-passport-modal" // ADD
import { useSwipeable } from "react-swipeable"
// First, add the import for the new modal component at the top of the file
import NewAlbumModal from "@/components/modals/new-album-modal" // ADD
// Add the import for AlbumDetailView at the top of the file
...
const PassportTabRedesign = ({ currentLanguage, onOpenModal }: PassportTabProps) => {
 ...
 // Update the state to track flip animation
 const [isFlipped, setIsFlipped] = useState(false)
 // Add a new state to track if the visa page is being viewed
 const [viewingVisaPage, setViewingVisaPage] = useState(false)
 const [showNewPassportModal, setShowNewPassportModal] = useState(false) // ADD
 const [activeCity, setActiveCity] = useState(0)
 // Update the activePassports state to include the already active cities
 const [activePassports, setActivePassports] = useState(["seoul", "bangkok", "busan", "jeju", "newyork"])
 // Add a new state for the album modal and albums data
 const [showNewAlbumModal, setShowNewAlbumModal] = useState(false) // ADD
 // Add sample albums data
 const [albums, setAlbums] = useState([
   {
     id: "album1",
     name: currentLanguage === "en" ? "2025 Europe Trip" : "2025년 유럽 여행",
     coverImage: "/images/guellpark.png",
     itemCount: 8,
   },
   {
     id: "album2",
     name: currentLanguage === "en" ? "Favorite Indie Bookstores" : "내가 사랑한 독립서점들",
     coverImage: "/images/goyoseosa.jpg",
     itemCount: 12,
   },
   {
     id: "album3",
     name: currentLanguage === "en" ? "Seoul Cafe Hopping" : "서울 카페 유랑",
     coverImage: "/images/cafeonion.png",
     itemCount: 5,
   },
 ]) // ADD

 // Add a new state to track the selected album
 ...
 const cities = [
   {
     ...
     image: "/images/cover_seoul.png", // Updated to highlight gold
     stamps: 24,
     totalStamps: 35,
   ...
   },
   {
     ...
     country: currentLanguage === "en" ? "South Korea" : "대한민국",
     image: "/images/cover_busan.png", // Updated to forest green
     stamps: 8,
     totalStamps: 20,
   ...
   },
   {
     ...
     image: "/images/cover_bangkok.png", // Updated to highlight gold
     stamps: 12,
     totalStamps: 28,
   ...
   },
   {
     ...
     image: "/images/cover_newyork.png", // Updated to forest green
     stamps: 3,
     totalStamps: 25,
   ...
   },
   {
     ...
     country: currentLanguage === "en" ? "South Korea" : "대한민국",
     image: "/images/cover_jeju.png", // Updated to highlight gold
     stamps: 0,
     totalStamps: 18,
 ...
 const handleAddNewPassport = () => {
   triggerHapticFeedback(hapticPatterns.medium)
   setShowNewPassportModal(true) // ADD
 }

 // Add a function to handle adding a new album
 ...
 // Add a function to open the new album modal
 ...
 const handleToggleIdentityPage = () => {
   ...
   // After animation completes, update the view state
   setTimeout(() => {
     setViewingIdentity(!viewingIdentity)
   }, 600) // Half the animation duration
 }

 // Add a function to handle toggling the visa page view
 const handleToggleVisaPage = () => {
   triggerHapticFeedback(hapticPatterns.medium)
   setViewingVisaPage(!viewingVisaPage)
 }

 // Enhance swipe handlers for better touch response
 ...
 // Add a new function to render the welcome message page
       ...
       <div className="relative z-10">
         ...
         <div className="mb-6 text-sm text-deep-navy">
           <p className="mb-4">
             ...
             소중한 순간들을 수집하고, 진정한 경험으로 이 페이지들을 채������, 당신만의 고유하고 생생한 여정의 이야기를
         ...
         <div className="mt-8 pt-4 border-t border-gray-300 flex justify-between items-center">
           ...
           <div className="w-24 h-24 relative">
             <Image
               src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/seal-Vu4SXwNaOb0O4eRtv65J5WBAnO2Tmo.png"
 ...
 // Add a function to handle album selection
 const handleAlbumSelect = (album: { id: string; name: string; coverImage: string; itemCount: number }) => {
   triggerHapticFeedback(hapticPatterns.medium)
   setSelectedAlbum(album)
 }

 // Add a function to handle going back from album detail view
 ...
 // Update the return statement to conditionally render the AlbumDetailView
 // Add this check right after the selectedCity check (before the return statement)
 if (selectedAlbum) {
   return (
     <AlbumDetailView
       ...
       {!isPassportOpen ? (
         ...
         >
           <div className="relative w-full max-w-md mx-auto mb-6">
             ...
             >
               ...
               <div className="absolute inset-0 overflow-hidden rounded-lg">
                 <img
                   src="/images/cover-new-classic.png" // Updated to classic blue
                   alt="Passportr main cover"
                   className="absolute inset-0 w-full h-full object-cover"
                   style={{ opacity: 0.85 }}
                 />
                 <div className="absolute inset-0 bg-gradient-to-t from-transparent via-transparent to-deep-navy opacity-30"></div>
               </div>
           ...
           <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
             <h3 className="text-lg font-bold serif-font mb-3">
               {currentLanguage === "en" ? "Global Journey" : "글로벌 여정"}
             </h3>
             ...
             <div className="flex justify-between text-sm mb-2">
               ...
               <span className="font-medium">
                 ...
                 {cities.reduce((acc, city) => acc + city.totalStamps, 0)}{" "}
                 {currentLanguage === "en" ? "Stamps" : "스탬프"}
               </span>
             </div>
             ...
             <div className="grid grid-cols-2 gap-3">
               <div className="bg-light-sand p-3 rounded-lg text-center">
                 <p className="text-xs text-stone-gray mb-1">
                   {currentLanguage === "en" ? "Cities Visited" : "방문한 도시"}
                 </p>
                 <p className="text-xl font-bold">{cities.filter((city) => city.stamps > 0).length}</p>
               </div>
               <div className="bg-light-sand p-3 rounded-lg text-center">
                 <p className="text-xs text-stone-gray mb-1">
                   {currentLanguage === "en" ? "Explorer Level" : "탐험가 레벨"}
                 </p>
         ...
         >
           ...
           {viewingIdentity ? (
             <div className="passport-identity-container">
...
const PassportTabRedesign = ({ currentLanguage, onOpenModal }: PassportTabProps) => {
  ...
  const [albums, setAlbums] = useState([
    {
      id: "album1",
      name: currentLanguage === "en" ? "Seoul Cafe Hopping '25" : "서울 카페 유랑",
      coverImage: "/images/seoulcafetour.png",
      itemCount: 8,
    },
    {
      id: "album2",
      name: currentLanguage === "en" ? "Weekend Getaways" : "주말여행",
      coverImage: "/images/album-cover-red.jpeg",
      itemCount: 12,
    },
    {
      id: "album3",
      name: currentLanguage === "en" ? "2025 Europe Trip" : "2025년 유럽 여행",
      coverImage: "/images/2025europe.png",
      itemCount: 5,
    },
  ]) // ADD
